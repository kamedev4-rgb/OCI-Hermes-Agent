...
- Current summary output preference: generate/store `title`, `summary_short`, and `summary_long` in Japanese. `summary_long` should be Markdown with `# AI詳細要約`, `## 概要`, `## 重要ポイント`, `## 詳細`, and `## 確認したい観点`. `summary_short` remains a 2–3 sentence list-card excerpt. When Gemini is available, use it for Japanese title/summaries; local fallback should also emit this structure but cannot translate English source text.
- Next planned Clipping Note section: implement a proper Markdown parser/rendering stack (`react-markdown`, `remark-gfm`, `rehype-sanitize`) and image support. A shared note for the next session exists in `SaaS/Clipping Note` titled `Clipping Note 次セッション: Markdownパーサ化と画像対応` (ID `01KR0RWEVDA9XAAJ5QM3635RF1`).
