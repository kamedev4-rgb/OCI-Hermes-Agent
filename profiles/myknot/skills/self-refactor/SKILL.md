...
現在の Git ルートは **常に `~/.hermes/`** として扱う。
`hermes-agent/` は別 repo ではなく、root repo 配下のディレクトリである。

- profile 設定・SOUL・profile-local skills を変えるときも
- `~/.hermes/hermes-agent/` 配下の Hermes 本体コードを変えるときも

**Git コマンドは `cd ~/.hermes` で実行する。**

`BOOT.md` は再起動引き継ぎ用の一時ファイルなので、コミットに含めない。

```bash
cd ~/.hermes && git add -A && git restore --staged BOOT.md 2>/dev/null || true
cd ~/.hermes && git commit -m "refactor: <目的>（修正前）" || true
```
...
```bash
cd ~/.hermes && git pull origin master
```
...
```bash
cd ~/.hermes && git add -A && git restore --staged BOOT.md 2>/dev/null || true
cd ~/.hermes && git commit -m "fix: <目的>（修正完了）" && git push origin master
```
