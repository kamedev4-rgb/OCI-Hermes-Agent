...
def _sync_skills_to_index(
    skills: List[Dict[str, Any]], *, skills_dir: Path | None = None, source: str = "local"
) -> None:
    """Best-effort sync of discovered skill metadata into the SQLite skill index."""
    if not skills:
        return
    try:
        from tools.skill_index_db import SkillIndexDB, get_skill_index_db_path

        db = SkillIndexDB(get_skill_index_db_path(skills_dir or SKILLS_DIR))
        for skill in skills:
            db.upsert_skill(
                {
                    ...
                    "enabled": not _is_skill_disabled(str(skill.get("name") or skill.get("skill_name") or "")),
                }
            )
    except Exception as e:
        logger.debug("Skill index sync skipped: %s", e)
...
def skills_list(category: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        _sync_skills_to_index(all_skills, skills_dir=SKILLS_DIR)
            ...
            db.upsert_skill(
                {
                    ...
                    "category": _get_category_from_path(skill_md),
                    "description": frontmatter.get("description", "") or result.get("description", ""),
                    "source": "local",
                    "skill_dir": str(skill_dir) if skill_dir else None,
                    "skill_path": str(skill_md),
                    "tags": tags,
                    "enabled": not _is_skill_disabled(skill_name),
                }
            )
            db.record_usage(
                skill_name=skill_name,
                session_id=os.getenv("HERMES_SESSION_ID") or task_id,
                platform=os.getenv("HERMES_PLATFORM") or os.getenv("HERMES_SESSION_PLATFORM"),
                trigger="skill_view",
            )
        except Exception as e:
            logger.debug("Skill usage tracking skipped for %s: %s", skill_name, e)
