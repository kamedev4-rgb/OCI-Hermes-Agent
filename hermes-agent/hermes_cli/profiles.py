...
def _profile_mem0_dbname(profile_name: str) -> str:
    """Return a PostgreSQL-safe Mem0 database name for a profile."""
    return profile_name.replace("-", "_")


def _load_json_file(path: Path) -> dict:
    """Best-effort JSON loader returning {} on failure/missing file."""
    if not path.exists():
        return {}
    try:
...
def _resolve_mem0_template(source_dir: Optional[Path] = None) -> dict:
    """Resolve a mem0 template from the source profile or hermes root."""
    candidates = []
    if source_dir is not None:
        candidates.append(source_dir / "mem0.json")
    candidates.append(_get_default_hermes_home() / "mem0.json")
    for candidate in candidates:
        cfg = _load_json_file(candidate)
        if cfg:
            return cfg
    return {}


def _write_profile_mem0_config(profile_dir: Path, profile_name: str, source_dir: Optional[Path] = None) -> Optional[Path]:
    """Write a profile-local mem0.json derived from the source/global template."""
    template = _resolve_mem0_template(source_dir)
    if not template:
        return None

    template["pg_dbname"] = _profile_mem0_dbname(profile_name)
    template["agent_id"] = profile_name

    out_path = profile_dir / "mem0.json"
    out_path.write_text(json.dumps(template, indent=2) + "\n", encoding="utf-8")
    return out_path


def _ensure_profile_mem0_database(profile_dir: Path, profile_name: str) -> None:
    """Best-effort creation of the profile's dedicated Mem0 database."""
    cfg = _load_json_file(profile_dir / "mem0.json")
    dbname = str(cfg.get("pg_dbname", "")).strip()
    if not dbname:
        return

    host = str(cfg.get("pg_host", "localhost")).strip() or "localhost"
    port = int(cfg.get("pg_port", 5432) or 5432)
    user = str(cfg.get("pg_user", "")).strip()
    password = str(cfg.get("pg_password", "")).strip()
    admin_db = str(cfg.get("pg_admin_db", "postgres")).strip() or "postgres"
    if not user:
        return
    ...
    try:
        ...
        with conn, conn.cursor() as cur:
            ...
            if not exists:
                ...
                print(f"✓ Created Mem0 database for profile '{profile_name}': {dbname}")
    except Exception as e:
        print(f"⚠ Could not create Mem0 database '{dbname}' for profile '{profile_name}': {e}")
...
def create_profile(
    ...
    # Seed a profile-local mem0.json and dedicated PostgreSQL database when a
    # mem0 template exists. This keeps named profiles isolated by default even
    # when they inherit their memory settings from the root profile.
