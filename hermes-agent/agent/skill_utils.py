...
DEFAULT_SKILL_RETRIEVAL_SETTINGS: Dict[str, Any] = {
    "enabled": True,
    "prompt_mode": "shortlist",
    "shortlist_limit": 10,
    "pinned": [
        "hermes-agent",
        "writing-plans",
...
def get_skill_retrieval_settings() -> Dict[str, Any]:
    """Read lightweight skill retrieval settings from config.yaml with defaults."""
    config_path = get_config_path()
    settings = dict(DEFAULT_SKILL_RETRIEVAL_SETTINGS)
    if not config_path.exists():
        return settings
    try:
        parsed = yaml_load(config_path.read_text(encoding="utf-8"))
    except Exception as e:
        logger.debug("Could not read skill retrieval config %s: %s", config_path, e)
        return settings
    if not isinstance(parsed, dict):
        return settings
    skills_cfg = parsed.get("skills")
    ...
    if "enabled" in retrieval_cfg:
        settings["enabled"] = bool(retrieval_cfg.get("enabled"))
    prompt_mode = str(retrieval_cfg.get("prompt_mode") or "").strip().lower()
    if prompt_mode in {"legacy", "shortlist"}:
        settings["prompt_mode"] = prompt_mode
    try:
        limit = int(retrieval_cfg.get("shortlist_limit", settings["shortlist_limit"]))
        settings["shortlist_limit"] = max(1, limit)
    except Exception:
        pass
    pinned = retrieval_cfg.get("pinned")
    if pinned is not None:
        settings["pinned"] = [str(v).strip() for v in pinned if str(v).strip()] if isinstance(pinned, list) else []
    return settings
