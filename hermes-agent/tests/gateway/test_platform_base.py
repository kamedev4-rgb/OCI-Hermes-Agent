...
class TestExtractMedia:
    ...
    def test_media_tag_rejects_placeholder_paths(self):
        content = "MEDIA:<screenshot_path>\nMEDIA:{screenshot_path}\nAfter"
        media, cleaned = BasePlatformAdapter.extract_media(content)
        assert media == []
        assert "MEDIA:" not in cleaned
        assert "After" in cleaned

    def test_source_checks_media_file_exists_before_sending(self):
        import inspect

        source = inspect.getsource(BasePlatformAdapter._process_message_background)
        assert "os.path.isfile(media_path)" in source or "Path(media_path).is_file()" in source
