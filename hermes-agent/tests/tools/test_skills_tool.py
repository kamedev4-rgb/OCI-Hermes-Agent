...
class TestSkillsList:
    ...
    def test_syncs_listed_skills_to_index_db(self, tmp_path, monkeypatch):
        ...
        row = SkillIndexDB(tmp_path / "skill_index.db").get_skill("alpha")
        assert row is not None
        assert row["description"] == "Description for alpha."
...
class TestSkillView:
    ...
    def test_records_usage_for_loaded_skill(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        skills_dir = tmp_path / "skills"
        with patch("tools.skills_tool.SKILLS_DIR", skills_dir):
        ...
        row = SkillIndexDB(tmp_path / "skill_index.db").get_skill("my-skill")
        assert row is not None
