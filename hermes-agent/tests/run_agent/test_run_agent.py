...
class TestCredentialPoolRecovery:
    ...
    def test_extract_api_error_context_uses_resets_in_seconds_when_present(self, agent):
        response = SimpleNamespace(headers={})
        error = SimpleNamespace(
            body={
                "error": {
                    ...
                    "resets_in_seconds": 304,
                }
            },
            response=response,
        )

        with patch("run_agent.time.time", return_value=1000.0):
            context = agent._extract_api_error_context(error)

        assert context["message"] == "The usage limit has been reached"
        assert context["reset_at"] == 1304.0
