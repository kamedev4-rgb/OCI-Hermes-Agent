...
class AIAgent:
    ...
    def _codex_quota_response_hook(self, response: Any) -> None:
        """Capture Codex quota headers without altering prompt content."""
        if self.api_mode != "codex_responses":
            return
        try:
            request_url = str(getattr(getattr(response, "request", None), "url", "") or "")
            if "chatgpt.com/backend-api/codex" not in request_url:
                return
            from agent.codex_quota import process_codex_headers

            process_codex_headers(
                dict(getattr(response, "headers", {}) or {}),
                access_token=str(self.api_key or ""),
                source=display_hermes_home(),
            )
        except Exception as exc:
            logger.debug("Codex quota capture skipped: %s", exc)
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        existing_http_client = client_kwargs.get("http_client")
        if existing_http_client is not None and self.api_mode == "codex_responses":
            try:
                hooks = getattr(existing_http_client, "event_hooks", None)
                if isinstance(hooks, dict):
                    hooks.setdefault("response", [])
                    if self._codex_quota_response_hook not in hooks["response"]:
                        hooks["response"].append(self._codex_quota_response_hook)
            except Exception:
                pass
        if "http_client" not in client_kwargs:
            try:
                ...
                event_hooks = None
                if self.api_mode == "codex_responses":
                    event_hooks = {"response": [self._codex_quota_response_hook]}
