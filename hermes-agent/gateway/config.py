...
class GatewayConfig:
    ...
    # Auto-resume prior thread sessions after idle/daily auto-reset.
    # Disabled by default: only opt-in deployments should automatically
    # stitch a reset thread back onto its prior transcript.
    auto_resume_thread_sessions: bool = False
    auto_resume_thread_platforms: List[str] = field(default_factory=lambda: ["discord"])
    auto_resume_thread_max_age_hours: int = 168
    auto_resume_thread_min_messages: int = 4
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "auto_resume_thread_sessions": self.auto_resume_thread_sessions,
            "auto_resume_thread_platforms": list(self.auto_resume_thread_platforms),
            "auto_resume_thread_max_age_hours": self.auto_resume_thread_max_age_hours,
            "auto_resume_thread_min_messages": self.auto_resume_thread_min_messages,
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        auto_resume_thread_sessions = _coerce_bool(
            data.get("auto_resume_thread_sessions"),
            False,
        )
        auto_resume_thread_platforms = data.get("auto_resume_thread_platforms", ["discord"])
        if not isinstance(auto_resume_thread_platforms, list):
            auto_resume_thread_platforms = ["discord"]
        auto_resume_thread_platforms = [str(p) for p in auto_resume_thread_platforms if p]
        try:
            auto_resume_thread_max_age_hours = int(data.get("auto_resume_thread_max_age_hours", 168))
        except (TypeError, ValueError):
            auto_resume_thread_max_age_hours = 168
        try:
            auto_resume_thread_min_messages = int(data.get("auto_resume_thread_min_messages", 4))
        except (TypeError, ValueError):
            auto_resume_thread_min_messages = 4
        ...
        return cls(
            ...
            auto_resume_thread_sessions=auto_resume_thread_sessions,
            auto_resume_thread_platforms=auto_resume_thread_platforms,
            auto_resume_thread_max_age_hours=auto_resume_thread_max_age_hours,
            auto_resume_thread_min_messages=auto_resume_thread_min_messages,
        )
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if "auto_resume_thread_sessions" in yaml_cfg:
                gw_data["auto_resume_thread_sessions"] = yaml_cfg["auto_resume_thread_sessions"]

            if "auto_resume_thread_platforms" in yaml_cfg:
                gw_data["auto_resume_thread_platforms"] = yaml_cfg["auto_resume_thread_platforms"]

            if "auto_resume_thread_max_age_hours" in yaml_cfg:
                gw_data["auto_resume_thread_max_age_hours"] = yaml_cfg["auto_resume_thread_max_age_hours"]

            if "auto_resume_thread_min_messages" in yaml_cfg:
                gw_data["auto_resume_thread_min_messages"] = yaml_cfg["auto_resume_thread_min_messages"]
