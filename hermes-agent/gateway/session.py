...
class SessionEntry:
    ...
    # Optional pointer to the just-expired session that can be automatically
    # restored for supported thread platforms (e.g. Discord) after idle/daily
    # reset. Persisted so a restart between reset and follow-up message does
    # not lose the candidate.
    resume_candidate_session_id: Optional[str] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "resume_candidate_session_id": self.resume_candidate_session_id,
        }
        if self.origin:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionEntry":
        ...
        return cls(
            ...
            resume_candidate_session_id=data.get("resume_candidate_session_id"),
        )
...
class SessionStore:
    ...
    def get_auto_resume_target(
        self,
        entry: SessionEntry,
        source: SessionSource,
    ) -> Optional[str]:
        """Return a previous session_id to auto-resume for supported thread resets."""
        cfg = self.config
        if not getattr(cfg, "auto_resume_thread_sessions", False):
            return None
        platform_name = source.platform.value if source.platform else ""
        if platform_name not in getattr(cfg, "auto_resume_thread_platforms", []):
            return None
        if source.chat_type != "thread" or not source.thread_id:
            return None
        if not getattr(entry, "was_auto_reset", False):
            return None
        if getattr(entry, "auto_reset_reason", None) not in ("idle", "daily"):
            return None
        candidate = getattr(entry, "resume_candidate_session_id", None)
        if not candidate or candidate == entry.session_id:
            return None

        max_age_hours = max(0, int(getattr(cfg, "auto_resume_thread_max_age_hours", 168) or 0))
        if max_age_hours:
            age = _now() - entry.created_at
            if age > timedelta(hours=max_age_hours):
                return None

        history = self.load_transcript(candidate)
        min_messages = max(1, int(getattr(cfg, "auto_resume_thread_min_messages", 4) or 1))
